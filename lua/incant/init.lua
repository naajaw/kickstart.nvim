...
local p_dark = {
  ...
  bg0 = "#1d2021", -- Primary editor background (Normal bg)
  bg1 = "#282828", -- Slightly raised surface: cursorline, selected items
  bg2 = "#3c3836", -- Floating windows, popups (NormalFloat bg)
  bg3 = "#504945", -- Borders, inactive separators
  bg4 = "#665c54", -- Subtle UI accents, column rulers

  -- Foregrounds (brightest → dimmest)
  fg0 = "#fbf1c7", -- Primary text (Normal fg)
  fg1 = "#ebdbb2", -- Secondary text: comments feel too bright at fg0
  fg2 = "#d5c4a1", -- Muted text: line numbers, inactive statusline
  fg3 = "#bdae93", -- Very dim: fold markers, end-of-buffer tildes

  -- Accent colors — used for syntax and UI highlights
  red = "#ffd3cb", -- Errors, deletions, keywords like `return`/`break`
  orange = "#fbd6bf", -- Numbers, constants, warnings
  yellow = "#eadeb9", -- Strings (warm tone), TODO comments
  green = "#d1e7c5", -- Added lines in diffs, successful diagnostics
  teal = "#c3ead3", -- Built-in types, special identifiers
  cyan = "#b9eae6", -- Function calls, member access
  blue = "#bae7f7", -- Keywords, statements, links
  purple = "#dedaff", -- Types, constructors, attributes/decorators
  magenta = "#f3d3ef", -- Search matches, flash/jump labels (high contrast)
...
local p_light = {
  -- Backgrounds (darkest → lightest)
  bg0 = "#FAF6F0", -- Primary editor background (Normal bg)
  bg1 = "#F5EBE0", -- Slightly raised surface: cursorline, selected items
  bg2 = "#F0E4D1", -- Floating windows, popups (NormalFloat bg)
  bg3 = "#FAF6F0", -- Borders, inactive separators
  bg4 = "#CBB4A4", -- Subtle UI accents, column rulers

  -- Foregrounds (brightest → dimmest)
  fg0 = "#000000", -- Primary text (Normal fg)
  fg1 = "#0F0B05", -- Secondary text: comments feel too bright at fg0
  fg2 = "#1F160A", -- Muted text: line numbers, inactive statusline
  fg3 = "#2E220F", -- Very dim: fold markers, end-of-buffer tildes
  fg4 = "#3D2D14",
  fg5 = "#4D3819",
  fg6 = "#5c431f",
  fg7 = "#6b4f24",
  ...
  -- Accent colors — used for syntax and UI highlights
  red = "#341d1d", -- Errors, deletions, keywords like `return`/`break`
  orange = "#331f14", -- Numbers, constants, warnings
  yellow = "#2b230f", -- Strings (warm tone), TODO comments
  green = "#1d2816", -- Added lines in diffs, successful diagnostics
  teal = "#132a1f", -- Built-in types, special identifiers
  cyan = "#0b2a2a", -- Function calls, member access
  blue = "#102733", -- Keywords, statements, links
  purple = "#242135", -- Types, constructors, attributes/decorators
  magenta = "#2c1e2f", -- Search matches, flash/jump labels (high contrast)
...
s.keyword = ext(s.plain, { fg = p.fg1, bold = true })
...
s.intrinsic = ext(s.plain, { fg = p.fg0, bold = true })
...
s.literal = ext(s.plain, { fg = p.orange, bold = true })
s.literalBold = ext(s.literal, { bold = true, underline = true }) -- @constant.builtin: nil, null, true, false

-- ── str ───────────────────────────────────────────────────────────────────────
-- String and character literals.
s.str = ext(s.plain, { fg = p.yellow, bold = true })
...
s.ctrl = ext(s.plain, { fg = p.red })
s.danger = ext(s.plain, { fg = p.red, bold = true }) -- hard syntax errors, FIXME badges
s.dangerItalic = ext(s.ctrl, { italic = true }) -- @variable.builtin: self, this, super

-- ── punctuation ───────────────────────────────────────────────────────────────
-- Delimiters, brackets, tag syntax: , ; . : ( ) [ ] { } < > /
-- Slightly dimmed so they don't compete visually with the tokens they separate.
s.punctuation = ext(s.plain, { fg = p.fg0 })

-- ── comment ───────────────────────────────────────────────────────────────────
-- Inline and block comments. Italic conventionally signals "not executable code".
s.comment = ext(s.plain, { fg = p.fg9, italic = true })
s.docComment = ext(s.comment, { fg = p.fg7 }) -- brighter: doc comments are API surface

-- ── commentWarn ───────────────────────────────────────────────────────────────
-- WARNING/HACK/WARN annotations inside comments. Colored text, no background.
s.commentWarn = ext(s.plain, { fg = p.none, bg = p.yellow, bold = true })
...
function M.setup()
  ...
  hi("Exception", s.ctrl) -- try, catch, throw, raise
  ...
  hi("@keyword.return", s.ctrl) -- return (signals exit, grouped with disruption)
  hi("@keyword.debug", s.ctrl) -- debugger, breakpoint
  hi("@keyword.exception", s.ctrl) -- try, catch, throw, raise, rescue
  ...
  hi("@comment.error", s.danger) -- FIXME, BUG, ERROR in comments
  hi("@comment.warning", s.commentWarn) -- WARNING, WARN, HACK in comments
  hi("@comment.todo", s.commentWarn) -- TODO, WIP
  hi("@comment.note", s.commentWarn) -- NOTE, INFO, HINT
  ...
  -- ── HTML / XML / JSX Tags ─────────────────────────────────────────────────
  hi("@tag", s.ctrl) -- <div>, <Component> (red: conventional for tag names)
